...
# Self-Reported (rename for clarity)
self_reported_chrono <- sleep_online_chrono %>%
  rename(self_reported_chronotype = chronotype_online)

# rMEQ-Based Chronotype from df (corrected scoring!)
df <- df %>%
  mutate(
    rmeq_chronotype = case_when(
      rmeq_score >= 22 & rmeq_score <= 25 ~ "Definitely morning",
...
# 4. Set factor levels in proper order
all_chrono_clean <- all_chrono_clean %>%
  mutate(
    ...
    chronotype_online = factor(chronotype_online,
                               levels = c("Definitely morning", "Rather morning", "Rather evening", "Definitely evening")),
    self_reported_chronotype = factor(self_reported_chronotype,
                                      levels = c("Definitely morning", "Rather morning", "Rather evening", "Definitely evening")),
...
library(ggalluvial)
alluvial_data <- all_chrono_clean %>%
  select(-self_reported_chronotype) %>%
